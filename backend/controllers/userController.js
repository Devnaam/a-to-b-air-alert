...
const calculateHealthMetrics = (stats, healthProfile) => {
  ...
  if (healthScore >= 90) {
    ...
    grade = 'D';
    message = 'Your exposure levels are concerning. Please prioritize healthier routes.';
  }
...
exports.deleteAccount = asyncHandler(async (req, res) => {
  ...
  const isPasswordValid = await User.verifyPassword(confirmPassword, user.password_hash);
...
exports.getActivitySummary = asyncHandler(async (req, res) => {
  ...
  const summary = {
    ...
    trends: {
      ...
      mostCommonRouteType: stats.healthyRoutePercentage > 50 ? 'healthiest' : 'fastest',
      exposureTrend: stats.avgExposure > 100 ? 'high' : stats.avgExposure > 50 ? 'moderate' : 'low'
    }
  };
