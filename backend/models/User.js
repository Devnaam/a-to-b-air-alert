...
class User {
	...
	static async findByEmail(email) {
		...
		if (user) {
			...
			try {
				user.health_profile =
					typeof user.health_profile === "string"
						? JSON.parse(user.health_profile || "{}")
						: user.health_profile || {};
				...
				console.warn("JSON parse error in findByEmail:", error);
				user.health_profile = {};
				user.preferences = {};
			}
		}
	...
	static async findById(id) {
		...
		if (user) {
			...
			try {
				...
				console.warn("JSON parse error in findById:", error);
				user.health_profile = {};
				user.preferences = {};
			}
		}
	...
	static async getStats(id) {
		const stats = await db("trips")
			.select(
				...
				db.raw(
					"COUNT(*) FILTER (WHERE route_type = 'healthiest') as healthy_route_count"
				),
				db.raw(
